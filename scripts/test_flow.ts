...
async function test() {
  console.log('--- INICIANDO PRUEBA DE FLUJO DE DIÁLOGO ---');
  const ctx = await setupTestContext();
  const { supabase, orgId, contact, agentConfig } = ctx;
  const testPhone = '573000000000';
  const conversationId = await freshConversation(supabase, orgId, contact!.id);

  // Turno 1: Cliente indica volumen de forma indirecta
  await runQuery(supabase, orgId, testPhone, contact!.full_name, conversationId, agentConfig, 
    "hola necesito unos esferitos para regalarle a mis 28 empleados y unos dos para mi"
  );

  // Turno 2: Cliente pide recomendación de bonitos y económicos
  await runQuery(supabase, orgId, testPhone, contact!.full_name, conversationId, agentConfig, 
    "el color no importa que me puedes recomendar que sean bonitos y economicos"
  );

  // Turno 3: Cliente pregunta por los precios
  await runQuery(supabase, orgId, testPhone, contact!.full_name, conversationId, agentConfig, 
    "que precio tienen?"
  );
